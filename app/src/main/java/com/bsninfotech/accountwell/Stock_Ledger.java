...
public class Stock_Ledger extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_stock__ledger);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setDisplayShowHomeEnabled(true);
        TypefaceUtil fontChanger = new TypefaceUtil(getAssets(), "fonts/" + ServerApi.FONT_DASHBOARD);
        fontChanger.replaceFonts((LinearLayout)findViewById(R.id.layout_stockledger));
        SpannableString str = new SpannableString("Stock Ledger Report");
        ...
        list_cashsummary = findViewById(R.id.list_stock_ledger);
        cashlist = new ArrayList<>();
        for (int i = 0; i < compname.length; i++) {
            helper = new BankSummary_Helper(compname[i],compnadd[i],crdr[i]);
            cashlist.add(helper);
        }
        StockLedger_Adapter adapter = new StockLedger_Adapter(Stock_Ledger.this, R.layout.item_stockledger, cashlist);
